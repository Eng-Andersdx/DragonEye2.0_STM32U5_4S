 ...
 * -----------------------------------------------------------------------------
 * 1.7     | 2026-04-27 | R.Gopan   | Updated LED Colours for Ring Light
 * Notes:
...
#if __DRAGONEYE_LEDS_H != 0107	/* BSP LEDS REVISION v.1.7 */
...
void BSP_LED_SetRGBIntensity(uint8_t red_dutycycle, uint8_t green_dutycycle, uint8_t blue_dutycycle){
	...
	__HAL_TIM_SET_COMPARE(tim_handle, g_channel, (green_dutycycle*tim_handle->Init.Period/100)*0.4);
	__HAL_TIM_SET_COMPARE(tim_handle, b_channel, (blue_dutycycle*tim_handle->Init.Period/100));
}
